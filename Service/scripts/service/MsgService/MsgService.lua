...
function MsgService:GetSocketHandleList()
	...
	serverList.open = 	 handler(self,self.Server_ConnectClient)--完成
...
function MsgService:Command_OpenListen(source,conf)
	...
	self._socket = socketdriver.listen(address,port)-- 设置监听信息
	socketdriver.start(self._socket)--开始监听端口  
	skynet.error(string.format("Listen on %s:%d", address,port)) 
	return self._msgMediatorObj:OpenListenHandle(source)--源 和 数据
end 
...
function MsgService:RegisterCommand(commandTable) 
	commandTable.open =  handler(self,MsgService.Command_OpenListen) 
	commandTable.close =  handler(self,MsgService.Command_CloseListen)  
end  
